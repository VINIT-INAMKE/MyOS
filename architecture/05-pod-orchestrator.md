...
- **NeMo Agent Toolkit patterns:** Defense middleware (content guard, PII scanner, output verifier) runs as a filter stage managed by the Pod Orchestrator between cubelet outputs and IPC delivery. Token/cost profiling feeds into AV reinforcement loop. Trajectory evaluation assesses pod execution quality post-completion.
